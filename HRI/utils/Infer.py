...
# @profile
def infer_one_step_vectorise(model, valuation, num_constants, unifs, unifs_duo, permute_masks=None, num_predicates=None, num_rules=None, num_keep=None, masks=None):
    """
        Vectorised inference procedure.
        Args:
    ...
    assert list(valuation.size())==[num_predicates-1, num_constants,num_constants] #since has removed tgt
    assert list(unifs.size())==[num_predicates, model.num_body, num_rules]
    assert list(unifs_duo.size())==[num_predicates-1, num_predicates-1,num_rules-1] #since has removed tgt
    
    #SANITY CHECK
    if model.args.add_p0:
        assert model.depth_predicates[0]==0 and model.depth_predicates[1]==0 and model.depth_predicates[2]==0 #That True and False depth 0 and next one too
        assert (abs(torch.max(valuation[0][:,:]).item()-1)<0.01 and abs(torch.min(valuation[0][:,:]).item()-1)<0.01)#TRUE
        assert (abs(torch.max(valuation[1][:,:]).item()-0)<0.01 and abs(torch.min(valuation[1][:,:]).item()-0)<0.01)#FALSE
        if model.args.task_name not in ['GQA', 'MT_GQA'] and not (abs(torch.max(valuation[2][:,:]).item()-1)<0.01 and abs(torch.min(valuation[2][:,:]).item()-0)<0.01):#FIRST background predicate should be nor True nor False
    ...
    if model.args.with_permutation:
        ...
        mask_permute_1= permute_masks[0].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat((1,num_predicates-1, num_constants,num_constants))
        #mask: if permute (or not) second body in rule
        mask_permute_2= permute_masks[1].unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat((1,num_predicates-1, num_constants,num_constants))
        assert list(mask_permute_1.size())==[num_rules-1,num_predicates-1, num_constants,num_constants] and list(mask_permute_2.size())==[num_rules-1,num_predicates-1, num_constants,num_constants]

    #mask for rule type C, scaled to size (num_pred-1,num_pred-1,num_cst,num_cst,num_cst)
    mask_rule_C= masks[0].unsqueeze(1).unsqueeze(1).unsqueeze(1).unsqueeze(0).unsqueeze(0).repeat((num_predicates-1,num_predicates-1,1,num_constants, num_constants,num_constants)) #mask which worth 1 if rule type c
    #mask for predicates arity 1, scaled to size (num_rules-1,num_cst,num_cst)
    mask_arity_1=masks[1].unsqueeze(1).unsqueeze(1).repeat((1,num_constants,num_constants))
    #mask for extended rule, scaled to size (num_rules-1,num_cst,num_cst)
    mask_extended_rule=masks[2].unsqueeze(1).unsqueeze(1).repeat((1,num_constants,num_constants))
    assert list(mask_rule_C.size())==[num_predicates-1,num_predicates-1,num_rules-1,num_constants, num_constants,num_constants]
    assert list(mask_arity_1.size())==[num_rules-1, num_constants,num_constants]
    assert list(mask_extended_rule.size())==[num_rules-1, num_constants,num_constants]

    # 1- init new valuations: here only non symbolic predicates. AT end would stack
    valuation_new=Variable(torch.zeros(valuation[num_keep:,].size()), requires_grad=True)
    assert list(valuation.shape)==[num_predicates-1, num_constants,num_constants]
    valuation_=valuation.unsqueeze(0).repeat((num_rules-1,1,1,1)) #size r-1, p-1,c,c
    ...
    if model.args.with_permutation:
        permuted_valuation=valuation_.transpose(2,3)#transpose val, #size r-1, p-1,c,c
        assert valuation_.shape==mask_permute_1.shape==mask_permute_2.shape and list(valuation_.shape)==[num_rules-1,num_predicates-1, num_constants,num_constants]
        val_1= mask_permute_1 * permuted_valuation + (1-mask_permute_1)* valuation_ #size r-1, p-1,c,c
        val_2= mask_permute_2 * permuted_valuation+ (1-mask_permute_2)* valuation_ #size r-1, p-1,c,c
    else: 
        val_1=valuation_
        val_2=valuation_
    fuzzy_and=fuzzy_and_vct(val_1,val_2, mode=model.args.fuzzy_and) #size r-1, p-1, p-1,c,c,c.
    assert list(fuzzy_and.shape)==[num_rules-1, num_predicates-1, num_predicates-1,  num_constants, num_constants,num_constants]
    score_aux=torch.einsum('pqr,rpqxyz->pqrxyz', unifs_duo,fuzzy_and) 
    assert list(score_aux.shape)==[num_predicates-1, num_predicates-1, num_rules-1, num_constants, num_constants,num_constants]
    ...
    score_c=score_c.unsqueeze(5).repeat((1,1,1,1,1,num_constants))#so r, p,p, x,y,z and may do min min z without affecting ot...
    if model.args.use_gpu and torch.cuda.is_available():
    ...
    assert list(score_aux.size())==[num_predicates-1,num_predicates-1,num_rules-1, num_constants,num_constants]

    #---5-- pool
    score_and=pool(score_aux, model.args.merging_and, dim=[0,1]) #size (r,c,c)
    
    #--6-- for AND ARITY 1 predicate: doing diag on final dimensions (X=Y) (KEEP DIM)
    #NOTE: in model.rules_arity, only aux predicates.
    score_and_arity_1 = torch.diagonal(score_and, dim1=1, dim2=2).unsqueeze(2).repeat((1,1, num_constants))
    ...
    if np.count_nonzero(extended_rules)>0:
        if model.args.scaling_OR_score=="square":
            ...
            score_b3=torch.einsum('pr,pxy->prxy', unifs[:-1,2,:-1], valuation)#size p-1,r-1, c,c
        score_b3_arity1 = torch.max(score_b3,  dim=3)[0].unsqueeze(3).repeat((1,1, 1, num_constants)) #keepdim=True, #size p-1,r-1, c,c
        mask_arity_1_=mask_arity_1.unsqueeze(0).repeat((num_predicates-1,1, 1,1))
        score_or=mask_arity_1_*score_b3_arity1 + (1-mask_arity_1_)*score_b3 #p-1, r-1,c,c
        score_or=pool(score_or, model.args.merging_or, dim=[0])#r-1,c,c
        assert list(score_or.shape)==[num_rules-1, num_constants,num_constants]
    ...
    if "OR2Inv" in model.rules_str:
        mask_ruleOR2Inv=torch.tensor([model.rules_str[r]=="OR2Inv" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        score_OR2Inv_1= pool(torch.einsum('pr,pxy->prxy', unifs[:-1,0,:-1], valuation), model.args.merging_or, dim=[0])#r-1,c,c
        score_OR2Inv_2= pool(torch.einsum('pr,pyx->prxy', unifs[:-1,1,:-1], valuation), model.args.merging_or, dim=[0])#r-1,c,c
        val_OR2Inv=fuzzy_or(score_OR2Inv_1,score_OR2Inv_2 , mode=model.args.fuzzy_or)#r,c,c

        if model.args.use_gpu and torch.cuda.is_available():
            mask_ruleOR2Inv = mask_ruleOR2Inv.cuda()

        valuation_new=mask_ruleOR2Inv*val_OR2Inv + (1-mask_ruleOR2Inv)*valuation_new

    #NOTE: OR1Inv:  F(X)<-exist Y  F(X,Y) or F(Y,X)
    if "OR1Inv" in model.rules_str:
        mask_ruleOR1Inv=torch.tensor([model.rules_str[r]=="OR1Inv" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        #existential quantifier on last cst before pool if arity 1
        score_OR1Inv_1=pool(torch.max(torch.einsum('pr,pxy->prxy', unifs[:-1,0,:-1], valuation),dim=3)[0] , model.args.merging_or, dim=[0])#
        score_OR1Inv_2=pool(torch.max(torch.einsum('pr,pyx->prxy', unifs[:-1,1,:-1], valuation),dim=3)[0], model.args.merging_or, dim=[0])#
        val_OR1Inv=fuzzy_or(score_OR1Inv_1,score_OR1Inv_2 , mode=model.args.fuzzy_or).unsqueeze(2).repeat(1,1,num_constants)

        if model.args.use_gpu and torch.cuda.is_available():
            mask_ruleOR1Inv = mask_ruleOR1Inv.cuda()

        valuation_new=mask_ruleOR1Inv*val_OR1Inv + (1-mask_ruleOR1Inv)*valuation_new
    
    #NOTE: OR2: F(X,Y)<-F(X,Y) or F(X,Y)
    if "OR2" in model.rules_str:
        mask_ruleOR2=torch.tensor([model.rules_str[r]=="OR2" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        score_OR2_1=pool(torch.einsum('pr,pxy->prxy', unifs[:-1,0,:-1], valuation), model.args.merging_or, dim=[0])#r-1,c,c
        score_OR2_2=pool(torch.einsum('pr,pxy->prxy', unifs[:-1,1,:-1], valuation), model.args.merging_or, dim=[0])#r-1,c,c
    ...
    if "OR1" in model.rules_str:
        mask_ruleOR1=torch.tensor([model.rules_str[r]=="OR1" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        score_OR1_1=pool(torch.max(torch.einsum('pr,pxy->prxy', unifs[:-1,0,:-1], valuation),dim=3)[0] , model.args.merging_or, dim=[0])#
        score_OR1_2=pool(torch.max(torch.einsum('pr,pxy->prxy', unifs[:-1,1,:-1], valuation),dim=3)[0], model.args.merging_or, dim=[0])#
        val_OR1=fuzzy_or(score_OR1_1,score_OR1_2 , mode=model.args.fuzzy_or).unsqueeze(2).repeat(1,1,num_constants)
        if model.args.use_gpu and torch.cuda.is_available():
    ...
    if "Inv" in model.rules_str:
        mask_ruleInv=torch.tensor([model.rules_str[r]=="Inv" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        val_Inv=pool(torch.einsum('pr,pyx->prxy', unifs[:-1,0,:-1], valuation),model.args.merging_or, dim=[0])#
    ...
    if "Rec2" in model.rules_str:
        mask_ruleRec2=torch.tensor([model.rules_str[r]=="Rec2" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        score_Rec2_1=pool(torch.max(torch.einsum('pr,pxz,rzy->prxyz', unifs[:-1,0,:-1], valuation, valuation[model.num_all_symbolic_predicates:,:,:]),dim=4)[0],model.args.merging_and, dim=[0])#rxy
        score_Rec2_2=pool(torch.einsum('qr,qxy->qrxy', unifs[:-1,1,:-1], valuation),model.args.merging_or, dim=[0])#rxy
        val_Rec2=fuzzy_or(score_Rec2_1,score_Rec2_2 , mode=model.args.fuzzy_or).unsqueeze(2).repeat(1,1,num_constants)

        if model.args.use_gpu and torch.cuda.is_available():
            mask_ruleRec2 = mask_ruleRec2.cuda()

        valuation_new=mask_ruleRec2*val_Rec2 + (1-mask_ruleRec2)*valuation_new
    
    #NOTE: Rec: P(X)<-(Q(X,Z) AND P(Z)) Or T(X,Y)
    if "Rec1" in model.rules_str:
        mask_ruleRec1=torch.tensor([model.rules_str[r]=="Rec1" for r in range(model.num_rules-1)]).double().unsqueeze(1).unsqueeze(1).repeat(1,num_constants,num_constants)
        #NOTE: here last dim there valuation[model.num_all_symbolic_predicates:,:,0] do not matter as unary...
        score_Rec1_1=pool(torch.max(torch.einsum('pr,pxz, rz->prxz', unifs[:-1,0,:-1], valuation, valuation[model.num_all_symbolic_predicates:,:,0]),dim=3)[0], model.args.merging_and, dim=[0]) #rx
        score_Rec1_2=pool(torch.max(torch.einsum('pr,pxz->prxz', unifs[:-1,1,:-1], valuation),dim=3)[0],model.args.merging_or, dim=[0])#rx
        val_Rec1=fuzzy_or(score_Rec1_1,score_Rec1_2 , mode=model.args.fuzzy_or).unsqueeze(2).repeat(1,1,num_constants)#rxy

        if model.args.use_gpu and torch.cuda.is_available():
            mask_ruleRec1 = mask_ruleRec1.cuda()

        valuation_new=mask_ruleRec1*val_Rec1 + (1-mask_ruleRec1)*valuation_new

    #---6. merge new and old valuation
    valuation_all = torch.cat([valuation[:num_keep].clone(), valuation_new.float()], dim=0)
    valuation_all = merge(valuation, valuation_all, mode=model.args.merging_val) 
    assert list(valuation_all.size())==[num_predicates-1, num_constants,num_constants]
...
def infer_tgt_vectorise(args, valuation, unifs, tgt_arity=2):
    ...
    if tgt_arity==1: #if tgt arity 1 need to use existential quantifier over first var
        score_tgt=torch.max(score_tgt, dim=2)[0].unsqueeze(2)#exist here on second dim
...
def infer_one_step(model, valuation, num_constants, unifs):
    ...
    valuation_new = [valuation[i].clone() for i in model.idx_background] + \
                    [Variable(torch.zeros(valuation[i].size())) for i in model.idx_aux]
    forbidden_idx=[]
    if model.args.add_p0:
        forbidden_idx=[0,1]
            ...
            if valuation[predicate].size()[1] == 1:
                ...
                for body1 in range(model.num_predicates): #here including p0 and p1       
                    if (model.depth_predicates[body1] <= max_depth) and not (model.rules_str[rule] == "TGT"):
                        for body2 in range(model.num_predicates):
                            if (model.depth_predicates[body2] <= max_depth):
                                unif = unifs[body1][rule] * unifs[body2][model.num_rules + rule]
                                # A00 F(X) <-- F(X,Z),F(Z,X)
                                ...
                                num = fuzzy_and(val_1, val_2, mode=model.args.fuzzy_and)
                                num = torch.max(num)  # for "exists", if size 2...
                                score_and = unif*num
                                #assert list(max_score.shape) in [[1],[]] and  list(score_and.shape) in [[1],[]]
                                max_score = merge(max_score, score_and, mode=model.args.merging_and)    
                    ...
                    #-----for tgt
                        ...
                        max_score = merge(max_score, score_tgt, mode=model.args.merging_tgt)
                ...
                if extended_rule:
                    for body3 in range(model.num_predicates):
                        ...
                        if model.args.hierarchical and (model.depth_predicates[body3] <= max_depth-1):
                            ...
                            score_or = unifs[body3][2 *model.num_rules + rule]*torch.max(val_3)
                            max_score_or = merge(max_score_or, score_or, mode=model.args.merging_or)
                    # merge two parts rules
                    max_score = fuzzy_or(max_score, max_score_or, mode=model.args.fuzzy_or)
                ...
                for o in range(num_constants):
                    ...
                    for body1 in range(model.num_predicates):
                            ...
                            for body2 in range(model.num_predicates):
                                if (model.depth_predicates[body2] <= max_depth):
                                    ...
                                    # "B01" F(X,Y) <-- F(X,Z),F(Y,Z)
                                        ...
                                        val_1 = valuation[body1][s, :].squeeze()
                                        val_2 = valuation[body2][o, :].squeeze()
                                    ...
                                    num = fuzzy_and(val_1, val_2, mode=model.args.fuzzy_and)  # scalar
                                    num = torch.max(num)
                                    score_and = unif*num
                                    max_score = merge(max_score, score_and, mode=model.args.merging_and)
                        ...
                        # tgt predicate being matched to 
                            ...
                            max_score = merge(max_score, score, mode=model.args.merging_tgt)
                    # ----extended rule case
                    if extended_rule:
                        for body3 in range(model.num_predicates):
                            if (model.depth_predicates[body3] <= max_depth-1):
                                if valuation[body3].size()[1] > 1:
                                    val_3 = valuation[body3][s, o]#TODO: CHECK whatbwant here
                                else:
                        ...
                        max_score = fuzzy_or(max_score, max_score_or, mode=model.args.fuzzy_or)
                    valuation_new[predicate][s, o] = merge(valuation[predicate][s, o], max_score, mode=model.args.merging_val)
...
def infer_one_step_campero(model, valuation, num_constants, unifs):
    """
    Here beware may be still several rules for one predicate
    """
    ...
    for m, predicate in enumerate(model.idx_aux):
        ...
        if model.args.hierarchical and ((model.args.recursivity=="none") or (not is_recursive)):#NB: if not hierarchical all depth at 0
            max_depth = depth-1
                    ...
                    for body1 in range(model.num_predicates):
                        ...
                        if (model.depth_predicates[body1] <= max_depth) and (model.rules_str[rule] == 1 or model.rules_str[rule] == 4 or model.rules_str[rule] == 12):
                            ...
                            # 12 F(X) <-- F(X,Z)
                                ...
                                val_1 = valuation[body1][s, :]
                            if torch.is_tensor(val_1):  # else body 1 do not match with template conditions
                                num = fuzzy_and(val_1, val_2, mode=model.args.fuzzy_and)  # scalar
                            ...
                            if (model.depth_predicates[body1] <= max_depth) and ((model.rules_str[rule] in [2] and valuation[body1].size()[1] == 1) or (model.rules_str[rule] in [13, 14] and valuation[body1].size()[1] > 1)):
                                for body2 in range(model.num_predicates):
                                    ...
                                    # 14 F(X) <-- F(X,Z), F(X,Z)
                                        ...
                                        val_1 = valuation[body1][s, :]
                                        val_2 = valuation[body2][s, :]
                                    if torch.is_tensor(val_1):  # else no match for body 2
                                        num = fuzzy_and(val_1, val_2, mode=model.args.fuzzy_and)  # scalar
                                        num = torch.max(num)
                                        score_and = unif*num
                                        max_score = merge(max_score, score_and, mode=model.args.merging_and)

                valuation_new[predicate][s, 0] = merge(valuation[predicate][s, 0], max_score, mode=model.args.merging_val)
                    ...
                    for rule in model.PREDICATES_TO_RULES[m]:#one or 2 rules
                        for body1 in range(model.num_predicates):
                            ...
                            if (model.depth_predicates[body1] <= max_depth) and (model.rules_str[rule] == 5 or model.rules_str[rule] == 8 or model.rules_str[rule] == 9):
                                ...
                                if torch.is_tensor(val_1):  # else body 1 do not match
                                    num = fuzzy_and(val_1, val_2, mode=model.args.fuzzy_and)  # scalar
                                    num = torch.max(num)
                                    score_and = unif*num
                                    max_score = merge(max_score, score_and, mode=model.args.merging_and)
