...
def soft_evaluation(model, task, print_results=False, num_iters=None, incremental=None):
    """
    Soft evaluation of a model (i.e., not symbolic, keeping all the soft weights), either incremental or not.
    Outputs:
        eval_acc_rate: evaluation accuracy Rate
    ...
    # ---1-- compute unifs
    unifs = get_unifs(model.rules.detach(), model.embeddings.detach(), args=model.args,
                      mask=model.hierarchical_mask, temperature=model.args.temperature_end, gumbel_noise=0.)

    # --2---evaluate
    if incremental:
        eval_acc_rate = evaluation(
            model, task=task, unifs=unifs, num_iters=num_iters, print_results=print_results)
    else:
        eval_acc_rate = incremental_evaluation(
            model, task=task, num_iters=num_iters,  unifs=unifs, print_results=print_results)
...
def symbolic_evaluation(model, task=None, num_iters=None, incremental=None, print_results=False, parameters=None, rules=None, task_idx=None, mode='test'):
    """
        Symbolic evaluation (once extracting a logical formula learned), may be incremental or not.

        Outputs:
            eval_acc_rate: evaluation accuracy rate when extracting the symbolic rule learned by the model.
            symbolic_path: list of symbolic rules used in the symbolic model, given in form of indices
            symbolic_success: bool, if symbolic success
    ...
    symbolic_unifs, symbolic_path, full_rules_str, permute_masks, symbolic_formula, rule_max_body_idx = extract_symbolic_model(
        model, parameters=parameters, rules=rules, predicates_labels=task.predicates_labels)

    # 2---run evaluation:
    if incremental:
        eval_acc_rate = incremental_evaluation(model, task=task, unifs=symbolic_unifs, num_iters=num_iters,
                                               permute_masks=permute_masks, print_results=print_results, task_idx=task_idx)
    else:
        if model.args.get_PR:
            eval_acc_rate, eval_precision, eval_recall = evaluation(
                model, task=task, unifs=symbolic_unifs, num_iters=num_iters, permute_masks=permute_masks, print_results=print_results, task_idx=task_idx)
        else:
            eval_acc_rate = evaluation(model, task=task, unifs=symbolic_unifs, num_iters=num_iters,
                                       permute_masks=permute_masks, print_results=print_results, task_idx=task_idx, mode=mode)
...
def evaluation(model, task=None, unifs=None, num_iters=None, permute_masks=None, print_results=False, task_idx=None, mode='test'):
    ...
    Outputs:
        eval_acc_rate: evaluation accuracy rate
        *precision: precision score, if model.args.get_PR
        *recall: recall score, if model.args.get_PR
    ...
    for epoch in range(num_iters):
        ...
        if model.args.task_name == 'GQA':
            ...
            valuation_eval_temp, bg_pred_ind_ls_noTF, target, num_constants, _ = task.data_generator.getData(
                mode=mode)
            valuation_eval = [torch.zeros(num_constants).view(-1, 1) if tp else torch.zeros(
                (num_constants, num_constants)) for tp in task.data_generator.if_un_pred]
            ...
            assert max(bg_pred_ind_ls_noTF) < model.num_background - \
                2*int(model.args.add_p0)
            ...
            #     )
            valuation_eval, target, num_constants = task.data_generator.getData(
                mode=mode)
        else:
            valuation_eval, target = task.data_generator.getData(num_constants)
        # ----2--add valuation other aux predicates
        valuation_eval = init_aux_valuation(
            model, valuation_eval, num_constants, steps=model.args.eval_steps)
        # --3---inference steps
        if model.args.use_gpu and torch.cuda.is_available():
            valuation_eval = valuation_eval.cuda()
            target = target.cuda()
        valuation_eval, valuation_tgt = model.infer(
            valuation_eval, num_constants, unifs=unifs, steps=model.args.eval_steps, permute_masks=permute_masks, task_idx=task_idx)
        # ----4---eval accuracy
        if model.args.get_PR:
        ...
        eval_acc_ls.append(torch.sum(torch.round(
            valuation_tgt) == target[:]).data.item())
        eval_acc_rate_ls.append(torch.sum(torch.round(
            valuation_tgt) == target[:]).data.item()/target.nelement())
    ...
    if print_results:
        ...
        if model.args.task_name in ['GQA', 'WN']:
            print(
                f'<percentage of correctly predicated targets>: {eval_acc_rate}')
        else:
            err_acc_rate = (target.nelement() -
                            eval_acc_mean) / target.nelement()
...
def incremental_evaluation(model, task=None, unifs=None, num_iters=None, permute_masks=None, print_results=False, task_idx=None):
    ...
    #NOTE: Incremental in the sense we keep adding constants.
    ...
    for num_constants in range(model.args.eval_st, model.args.eval_ed + 1, model.args.inc_eval_step):
        ...
        for epoch in range(num_iters):
            ...
            if model.args.use_gpu and torch.cuda.is_available():
                ...
                target = target.cuda()
            valuation_eval, valuation_tgt = model.infer(
                valuation_eval, num_constants, unifs, steps=model.args.eval_steps, permute_masks=permute_masks, task_idx=task_idx)

            # ---4----eval accuracy
            eval_acc.append(torch.sum(torch.round(
                valuation_tgt) == target[:]).data.item())
...
def predicates_evaluation(model, err_acc_rate):
    ...
    Outputs:
        utility_scores: utility scores of all the predicates, computed recursively (hierarchically), top down, using the unification scores
            (target predicate score 1)
        convergence_scores: convergence score of all the predicates, computed recursively (hierarchically), bottom up, using the unification scores
            (initial predicates score 1")
    ...
    # TODO background included in depth_predicates ?
    sorted_idx = depth_sorted_idx(depth_predicates)

    # ---get unifs
    unifs = get_unifs(model.rules.detach(), model.embeddings.detach(), args=model.args, mask=model.hierarchical_mask,
                      temperature=model.args.temperature_end, gumbel_noise=0.)  # size (num_predicates, 3*num_rules)

    # ----compute utility scores
    utility_scores = utility_evaluation(model, unifs, sorted_idx, err_acc_rate)

    # ----compute convergence scores
    convergence_scores = convergence_evaluation(model, unifs.view(
        model.num_predicates, model.num_body, model.num_rules), sorted_idx, err_acc_rate)
...
def utility_evaluation(model, unifs, sorted_idx, err_acc_rate):
    """
        Compute some utility scores top down for each predicate including symbolic ones and background ones.
        Target predicate having a utility score of 1.
        #NOTE: Look for each predicate if used in higher depth rules.
    """
    ...
    weighted_unifs = torch.max(unifs.view(
        (model.num_predicates, 3, model.num_rules)), dim=1)[0]

    assert list(weighted_unifs.shape) == [
        model.num_predicates, model.num_rules]

    # ---top down
    # reversed array here
    for depth, current_depth_idx in enumerate(sorted_idx[::-1]):
        ...
        if actual_depth < tgt_depth:
            ...
            if actual_depth > 0:
                utility_2 = torch.max(utility_1[:].unsqueeze(0).repeat(
                    (len(current_depth_idx), 1)) * weighted_unifs[p1:pn+1, r1:rn+1], dim=1)[0]
                # final :
            ...
            if actual_depth > 0:
                weighted_unifs[:, r1:rn+1] = utility_scores[p1:pn+1].unsqueeze(
                    0).repeat((model.num_predicates, 1)) * weighted_unifs[:, r1:rn+1]
...
def convergence_evaluation(model, unifs, sorted_idx, err_acc_rate):
    """
    Compute some convergence score, Bottom Up
    Initial predicates having a score of 1
    """
    ...
    for depth in range(len(sorted_idx)):
        for predicate in sorted_idx[depth]: # TODO: parallelize
            rule = predicate - all_symb  # corresponding rule
            if predicate in model.idx_soft_predicates:
                if model.rules_str[rule] == "TGT":
                    variance[predicate] = np.var(
                        unifs[:, 0, rule].numpy())  # only 1st coeff matter
                else:
    ...
    # convergence scores between 0 and 1.
    # TODO: test other formula
    convergence_scores = np.exp(-variance) / math.exp(0)
