...
class coreModel(nn.Module):
    ...
    def __init__(self, args,
        ...
        if self.args.task_name in ['MT_GQA','MT_WN']: 
            ...
            self.embeddings_intensions = nn.Parameter(init_predicates[self.num_background-2*int(self.args.add_p0):], requires_grad=True)
            ...
            self.embeddings_bgs = nn.Parameter(init_predicates[:self.num_background-2*int(self.args.add_p0)], requires_grad=False)
            self.embeddings_intensions = nn.Parameter(init_predicates[self.num_background-2*int(self.args.add_p0):], requires_grad=True)

        self.num_params=self.get_num_params()
        assert (self.args.softmax in ["softmax", "gumbel"]) or (self.args.clamp in ["param", "sim"])
        assert not ( self.args.softmax in ["softmax", "gumbel"] and self.args.clamp=="param")#do not clamp param if use softmax!

        #NOTE: TEMPORARY here to merge w/ progressive Model and use same procedure inference
        self.num_soft_predicates=self.num_rules
        self.idx_soft_predicates= self.idx_aux
        self.num_all_symbolic_predicates=self.num_background #here no other symb predicate
    ...
    def initialise(self):
        ...
        if self.args.unified_templates:
            ...
            if self.args.template_name=="campero" and len(self.pred_two_rules)>0:
                ...
                self.RULES_TO_PREDICATES, self.PREDICATES_TO_RULES=map_rules_to_pred(self.num_aux, self.idx_aux, self.num_rules, self.pred_two_rules)
            
            # NOTE: when template_name == 'new'
            else:
                self.num_aux=self.num_rules
            self.idx_background = [i for i in range(self.num_background)]
            self.idx_aux = [(self.num_background + i) for i in range(self.num_aux)]            
        ...
        if self.args.hierarchical and not self.args.unified_templates: #model_hierarchical
            ...
            self.depth_predicates = [0 for pred in self.idx_background] + self.depth_aux_predicates
        ...
        if self.args.hierarchical and self.args.unified_templates:
            self.hierarchical_mask=get_hierarchical_mask(self.depth_predicates, self.num_rules, self.num_predicates, self.num_body, self.rules_str, recursivity=self.args.recursivity)
        else:
...
# -----------------------------------------------------
    ...
    def train(self, task=None):
        ...
        print("Start training for {} iterations with {} constants ".format(self.args.num_iters,self.args.train_num_constants ))
        ...
        if self.args.criterion=="BCE":
            ...
            criterion = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(self.args.pos_weights_bce))
        else:
        ...
        for epoch in range(self.args.num_iters):
            ...
            if self.args.task_name=='GQA':
                ...
                valuation_init, bg_pred_ind_ls_noTF, target, num_constants, _ = self.data_generator.getData(mode='train')
                total_pred_ind_ls_noTF = bg_pred_ind_ls_noTF + [self.num_background-2*int(self.args.add_p0)+i for i in range(self.num_aux)]
                if self.args.add_p0:
                    total_pred_ind_ls_TF = [0, 1] + [x+2 for x in total_pred_ind_ls_noTF]
                else:
                    total_pred_ind_ls_TF = total_pred_ind_ls_noTF
                assert max(bg_pred_ind_ls_noTF) < self.num_background-2*int(self.args.add_p0)
            ...
            if self.args.use_noise:
                ...
                if self.args.task_name=='GQA':
                    valuation, valuation_tgt = self.infer(valuation, num_constants, unifs, steps=self.args.train_steps,
                                                          num_predicates=len(total_pred_ind_ls_TF), num_keep=len(bg_pred_ind_ls_noTF)+2*int(self.args.add_p0))
                else:
                    valuation, valuation_tgt = self.infer(valuation, num_constants, unifs, steps=self.args.train_steps)
            else:
            ...
            #---loss
            valuation_tgt = torch.clamp(valuation_tgt, 0, 1).type(torch.float32) #NOTE: necessary clamp here? 
            # pdb.set_trace()
            ...
            if self.args.reg_type == 0:
                ...
                loss = loss + self.args.reg1*torch.abs(flat_unifs).sum() + self.args.reg2*(flat_unifs*(1-flat_unifs)).sum()
            else:
                raise NotImplementedError
            ...
            if self.args.stop_training == "threshold":
                ...
                ave_loss = np.sum(losses[0-self.args.loss_interval:])/self.args.loss_interval
                ...
                if ave_loss < self.args.training_threshold and ave_loss > 0:
                    ...
                    if self.args.get_PR:
                        eval_acc_rate, precision, recall = evaluation(self, task=task, unifs=unifs, mode='valid', num_iters=eval_num_iters)
                        validation_precision[epoch] = precision
                        validation_recall[epoch] = recall
                        print(f'early stopping test with ave_loss = {ave_loss}, eval_acc = {eval_acc_rate}, precision = {precision}, recall = {recall}')
                    else:
                        eval_acc_rate = evaluation(self, task=task, unifs=unifs, mode='valid', num_iters=eval_num_iters)
                        print(f'early stopping test with ave_loss = {ave_loss}, eval_acc = {eval_acc_rate}')
                ...
                if epoch%self.args.eval_interval==0:
                    ...
                    if self.args.get_PR:
                        ...
                        validation_precision[epoch] = precision
                        validation_recall[epoch] = recall
                        print(f'early stopping test for epoch {epoch}, eval_acc = {eval_acc_rate}, precision = {precision}, recall = {recall}')
                    else:
                        eval_acc_rate = evaluation(self, task=task, unifs=unifs, mode='valid', num_iters=eval_num_iters)
                        print(f'early stopping test for epoch {epoch}, eval_acc = {eval_acc_rate}')
        ...
        if self.args.get_PR:
            train_acc_rate, train_precision, train_recall = evaluation(self, task=task, unifs=unifs, mode='test')
        else:
            train_acc_rate = evaluation(self, task=task, unifs=unifs, mode='test')
...
# -------------------------------------------------
    ...
    # @profile
    def infer(self, valuation, num_constants, unifs, steps=1, permute_masks=None, task_idx=None, num_predicates=None, num_keep=None):
        ...
        if self.args.vectorise:
            ...
            unifs_duo=torch.einsum('pr,qr->pqr', unifs_[:-1,0,:-1], unifs_[:-1,1,:-1]).view(-1,self.num_rules-1)#shape p-1, p-1, r-1 -- removed tgt from predicates
            if self.args.normalise_unifs_duo: #TODO: Which unifs ?
                unifs_duo=unifs_duo / torch.sum(unifs_duo, keepdim=True, dim=0)[0]
            unifs_duo=unifs_duo.view(num_predicates-1, num_predicates-1, self.num_rules-1)
            ...
            if self.args.vectorise:
                if self.args.infer_neo:
                    valuation = infer_one_step_vectorise_neo(self, valuation, num_constants, unifs_, unifs_duo, 
                                                             num_predicates=num_predicates, num_keep=num_keep)
                else:
                ...
                if self.args.template_name == "campero": # TODO: to be done
                    valuation = infer_one_step_campero(self, valuation, num_constants, unifs)
                elif self.args.template_name == "new":
                    valuation = infer_one_step(self, valuation, num_constants, unifs)
                else:
                    raise NotImplementedError
        
        #3---- tgt valuation computed at very end here
        if self.args.vectorise:
            valuation_tgt=infer_tgt_vectorise(self.args, valuation, unifs.view(num_predicates, self.num_body, self.num_rules), tgt_arity=self.rules_arity[-1])
        else:
...
# -------------------------------------------------
    def init_mtwn_training(self):
        ...
        if self.args.learn_wo_bgs:
            self.optimizer = torch.optim.Adam([
                {'params': [self.embeddings_intensions], 'lr': self.args.wn_lr_its},
    ...
    def initialise_training(self):
        ...
        if args.learn_wo_bgs:
            self.optimizer = torch.optim.Adam([
                {'params': [self.embeddings_intensions], 'lr': args.gqa_lr_its},
    ...
    def train_one_iter(self, valuation_init, bg_pred_ind_ls_noTF, target, num_constants, tgt_name):
        args = self.args
        # --------select embeddings
        total_pred_ind_ls_noTF = bg_pred_ind_ls_noTF + [self.num_background-2*int(args.add_p0)+i for i in range(self.num_aux)]
        if args.add_p0:
            total_pred_ind_ls_TF = [0, 1] + [x+2 for x in total_pred_ind_ls_noTF]
        else:
            total_pred_ind_ls_TF = total_pred_ind_ls_noTF
        ...
        if args.use_noise:
            ...
            emb_noise = self.head_noise_scale * torch.randn(embeddings_tmp.size())
            if args.use_gpu:
                rule_noise = rule_noise.cuda()
                emb_noise = emb_noise.cuda()
            noisy_rules = self.rules + rule_noise
        ...
        if self.args.use_noise:
            #------compute unifications score with noisy embeddings and rules
            unifs = get_unifs(noisy_rules, noisy_embeddings, args=args, mask=mask, temperature=self.temperature, gumbel_noise=self.gumbel_noise)
            #---- inference steps
            valuation, valuation_tgt = self.infer(valuation, num_constants, unifs, steps=args.train_steps,
                                                    num_predicates=len(total_pred_ind_ls_TF), num_keep=len(bg_pred_ind_ls_noTF)+2*int(args.add_p0))
        else:
            #------compute unifications score
            unifs = get_unifs(self.rules, embeddings_tmp, args=args, mask=mask, temperature=self.temperature, gumbel_noise=self.gumbel_noise)
            #---- inference steps
            valuation, valuation_tgt = self.infer(valuation, num_constants, unifs, steps=args.train_steps,
                                                    num_predicates=len(total_pred_ind_ls_TF), num_keep=len(bg_pred_ind_ls_noTF)+2*int(args.add_p0))
        ...
        if args.reg_type == 0:
            ...
            loss = loss + args.reg1*torch.abs(flat_unifs).sum() + args.reg2*(flat_unifs*(1-flat_unifs)).sum()
        else:
            raise NotImplementedError
