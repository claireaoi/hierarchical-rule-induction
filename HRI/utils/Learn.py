...
class Learn():
    ...
    def run(self):
        ...
        # do not use these rule structure for other...
        data_generator, background_predicates, intensional_predicates, rules_str = task.data_generator, task.background_predicates, task.intensional_predicates, task.campero_rules_str
        ...
        for i in range(args.num_runs):
            ...
            if args.log_loss:
                writer = SummaryWriter(
                    self.args.log_loss_dir + self.args.task_name + '/' + self.args.loss_tag + '/loss_'+str(i))
                writers = (writer,)
                if args.reg_type:
                    reg_writer = SummaryWriter(
                        self.args.log_loss_dir + self.args.task_name + '/' + self.args.loss_tag + '/reg_loss_'+str(i))
                    writers = writers + (reg_writer,)
            else:
                writers = None

            print(
                f"##################### task: {args.task_name}, run {i + 1} #####################")
            ...
            if args.get_PR:
                train_acc_rate, train_precision, train_recall, embeddings_temporal, unifs_temporal, losses, valuation, unifs, train_iters = model.train(
                    task=task)
            else:
            ...
            start_time = time.time()
            start_ptime = time.process_time()
            unifs = get_unifs(model.rules, model.embeddings, args=self.args,
                              mask=model.hierarchical_mask, temperature=model.args.temperature_end, gumbel_noise=0.)
            if self.args.inc_eval:
                eval_acc_rate = incremental_evaluation(
                    model, task=task, unifs=unifs)
            else:
                eval_num_iters = task.data_generator.dataset.len_test_file_ids if self.args.task_name == 'GQA' else self.args.num_iters_eval
                if args.get_PR:
                    eval_acc_rate, eval_precision, eval_recall = evaluation(
                        model, task=task, unifs=unifs, num_iters=eval_num_iters)
                else:
                    eval_acc_rate = evaluation(
                        model, task=task, unifs=unifs, num_iters=eval_num_iters, mode='test')
            ...
            print("eval time (time.process_time):",
                  round(end_ptime-start_ptime, 2))
            ...
            if print_results:
                ...
                if self.args.inc_eval:
                    num_eval_succ += 1 if sum(1 - np.array(eval_acc_rate)
                                              ) <= self.args.eval_threshold else 0
                else:
                    num_eval_succ += 1 if 1 - eval_acc_rate <= self.args.eval_threshold else 0

                print(f'======= unifs in run {i + 1}=======')
                print("unifs", unifs.view(model.num_predicates,
                ...
                print(
                    f'======= formula in run {i + 1} with task {self.args.task_name} and target {tgt_name}=======')
                ...
                if self.args.get_PR:
                    ...
                    print("Train precision {}, recall {}".format(
                        train_precision, train_recall))
                    print("Eval precision {}, recall {}".format(
                        eval_precision, eval_recall))
                    print("Symbolic eval precision {}, recall {}".format(
                        symbolic_eval_precision, symbolic_eval_recall))
                    print('')

                train_success_run = bool(
                    1 - train_acc_rate <= self.args.eval_threshold)
                eval_succ_run = bool(1 - eval_acc_rate <=
                                     self.args.eval_threshold)
                print(f'======= acc rate & succ in run {i + 1}=======')
                print("Train acc rate {}, Train succ of the run: {}".format(
                    train_acc_rate, train_success_run))
                print("Eval acc rate {}, Eval Succ of the tun: {}".format(
                    eval_acc_rate, eval_succ_run))
                print("Symbolic Eval acc rate {} Symbolic succ of the run: {}".format(
                    symbolic_eval_acc_rate, symbolic_success))
                print('')
        ...
        if print_results:
            ...
            print(
                f'### recap training_iter: max {max(train_iter_ls)}, min {min(train_iter_ls)}, mean {st.mean(train_iter_ls)}, stdev {st.pstdev(train_iter_ls)}')
            print('')

            # ----- time
            print("---------- time -----------")
            print("training_time_elapsed_ls (time.time):",
                  training_time_elapsed_ls)
            print("training_time_process_ls (time.process_time):",
                  training_time_process_ls)
            print(
            ...
            print(
                f'### recap training_time (time.process_time): max {round(max(training_time_process_ls),0)}, min {round(min(training_time_process_ls),0)}, mean {round(st.mean(training_time_process_ls),2)}, stdev {round(st.pstdev(training_time_process_ls),0)}')

            print("\nevaluation_time_elapsed_ls (time.time):",
                  evaluation_time_elapsed_ls)
            print("evaluation_time_process_ls (time.process_time):",
                  evaluation_time_process_ls)
            print(
            ...
            print(
                f'### recap evaluation_time (time.process_time): max {round(max(evaluation_time_process_ls),0)}, min {round(min(evaluation_time_process_ls),0)}, mean {round(st.mean(evaluation_time_process_ls),0)}, stdev {round(st.pstdev(evaluation_time_process_ls),0)}')
            print('')

            # ----- acc rate
            print(
                "---------- acc rate (percentage of correctly predicated targets) -----------")
            ...
            print(
                f'### recap eval_acc_rate: max {round(max(eval_acc_rate_ls),4)}, min {round(min(eval_acc_rate_ls),4)}, mean {round(st.mean(eval_acc_rate_ls),4)}, stdev {round(st.pstdev(eval_acc_rate_ls),4)}')
            if self.args.symbolic_evaluation:
                print(
                    f'### recap symbolic_acc_rate: max {round(max(symbolic_acc_rate_ls),4)}, min {round(min(symbolic_acc_rate_ls),4)}, mean {round(st.mean(symbolic_acc_rate_ls),4)}, stdev {round(st.pstdev(symbolic_acc_rate_ls),4)}')
            print('')

            # ----- succ
            print(
                "---------- succ (successful runs that predicate all targets correctly) -----------")
            print("### num_train_acc:", num_train_succ,
                  ",", num_train_succ / args.num_runs)
            print("### num_eval_acc:", num_eval_succ,
                  ',', num_eval_succ / args.num_runs)
            ...
            if self.args.get_PR:
                ...
                print(
                    f'### recap eval_precision_ls: max {round(max(eval_precision_ls),4)}, min {round(min(eval_precision_ls),4)}, mean {round(st.mean(eval_precision_ls),4)}, stdev {round(st.pstdev(eval_precision_ls),4)}')
                if self.args.symbolic_evaluation:
                    print(
                        f'### recap symbolic_precision_ls: max {round(max(symbolic_precision_ls),4)}, min {round(min(symbolic_precision_ls),4)}, mean {round(st.mean(symbolic_precision_ls),4)}, stdev {round(st.pstdev(symbolic_precision_ls),4)}')
                print('')
                ...
                print(
                    f'### recap eval_recall_ls: max {round(max(eval_recall_ls),4)}, min {round(min(eval_recall_ls),4)}, mean {round(st.mean(eval_recall_ls),4)}, stdev {round(st.pstdev(eval_recall_ls),4)}')
                if self.args.symbolic_evaluation:
                    print(
                        f'### recap symbolic_recall_ls: max {round(max(symbolic_recall_ls),4)}, min {round(min(symbolic_recall_ls),4)}, mean {round(st.mean(symbolic_recall_ls),4)}, stdev {round(st.pstdev(symbolic_recall_ls),4)}')
                print('')
            ...
            #     # Draw lines #magenta
            #     # train_sizes, needed?
            #     plt.plot(train_sizes[start:], train_mean[start:],
            #              color="#FF00FF",  label="Mean")
            #     # Draw bands
            #     plt.fill_between(train_sizes[start:], train_mean[start:] - train_std[start:],
            #                      train_mean[start:] + train_std[start:], color="#DDDDDD")
            #     # draw individual curves
            #     for i in range(args.num_runs):
            #         rgb = (random.random(), random.random(),
            #                random.random())  # random color
            #         plt.plot(train_sizes[start:], training_losses[i, start:], '--', color=rgb,  label="TrainA{} EvalA{} SymbS {}".format(
            #             round(train_accs[i], 3), round(eval_accs[i], 3), symbolic_successes[i]), alpha=0.3)
            #     # Create plot
            #     plt.title("Task {} Template {}.".format(
            #         args.task_name, args.template_set))
            #     plt.xlabel("Num Iterations"), plt.ylabel(
            #         "Training loss"), plt.legend(loc="best")
            #     plt.tight_layout()
            #     plt.show()
