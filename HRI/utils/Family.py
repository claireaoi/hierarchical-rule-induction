...
#     # return self.mul(self.father, self.daughter).max(axis=1)
    ...
    def get_sisters(self):
        return (self.mul(self.daughter, self.father) - np.eye(self._n)) > 0

    def get_brothers(self):
        return (self.mul(self.son, self.father) - np.eye(self._n)) > 0
    
    def get_son_of_father(self):
        # return the relations between fathers and sons
        return (self.son - self.mother.T) > 0

    def get_parents(self):
        return np.clip(self.father + self.mother, 0, 1)

    def get_grandfather(self):
        return self.mul(self.father, self.get_parents())

    def get_grandmother(self):
        return self.mul(self.mother, self.get_parents())
...
def randomly_generate_family(n, p_marriage=0.8, verbose=False):
    ...
    def add_couple(man, woman):
        ...
        rel[man, woman, 0] = 1  # husband
        rel[woman, man, 1] = 1  # wife
        if verbose:
    ...
    def add_child(parents, child, gender):
        ...
        rel[father, child, 2] = 1  # father
        rel[mother, child, 3] = 1  # mother
        if gender == 0:  # son
            rel[child, father, 4] = 1
            rel[child, mother, 4] = 1
        else:  # daughter
            rel[child, father, 5] = 1
            rel[child, mother, 5] = 1
        if verbose:
