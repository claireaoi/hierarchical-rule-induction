...
from utils.LearnMultiTasks_WN import LearnMultiTasks
...
parser.add_argument('--task_name', default='MT_WN', type=str, help='task name')  # multi-task WordNet
...
# extra arguments for WordNet

parser.add_argument('--wn_min_const_each', default=20, type=int, help='min number of constants inside one WN instance')
...
parser.add_argument('--wn_num_round', default=1, type=int, help='train through all target in one round')

# parser.add_argument('--wn_eval_all_embeddings', default=False, type=str2bool, help='If True, use all predicate mebeddings to get unifs; otherwise, only use relevant predicates in scene graph')
parser.add_argument('--wn_iter_per_round', default=1, type=int, help='# of training iterations per target per round')
...
parser.add_argument('--wn_valid_during_training', default=False, type=str2bool, help='True if conduct validation during training')
parser.add_argument('--wn_valid_round_interval', default=10, type=int, help='round interval for validation during training')
parser.add_argument('--wn_data_generator_tag', default='', type=str, help='tag for corresponding data generator, if gqa_eval_only=True')

# NOTE: this tgt_ls for debugging
parser.add_argument('--wn_tgt_ls', default=0, type=int, help='Decide which tgt_pred_ls to train for.')
parser.add_argument('--wn_lr_bgs', default=0.1, type=float, help='learning rate for background embeddings')
parser.add_argument('--wn_lr_its', default=0.1, type=float, help='learning rate for intensional embeddings')
parser.add_argument('--wn_lr_rules', default=0.1, type=float, help='learning rate')

parser.add_argument('--wn_eval_only', default=False, type=str2bool, help='skip training. load pretrained model and evaluate. Use the same hyperparameters as training the model, except this flag.')

parser.add_argument('--wn_eval_each_tgt', default=True, type=str2bool, help='evaluate precision & recall for each tgt model')
parser.add_argument('--wn_eval_each_ipp', default=3, type=int, help='(iteration per pred) maximum # evaluation instances during training for each pred for evaluation_all_tgt. If you want to evaluate on a whole dataset, you should set it to a huge number like 100000')
parser.add_argument('--wn_eval_all_tgt', default=True, type=str2bool, help='evaluate recall@1 & recall@5 for all tgt models')
parser.add_argument('--wn_eval_all_ipp', default=3, type=int, help='(iteration per pred) maximum # evaluation instances during training for each pred for evaluation_each_tgt. If you want to evaluate on a whole dataset, you should set it to a huge number like 100000')

parser.add_argument('--wn_eval_valid', default=True, type=str2bool, help='evaluate recall@1 & recall@5 for all tgt models')
parser.add_argument('--wn_eval_test', default=True, type=str2bool, help='evaluate recall@1 & recall@5 for all tgt models')


# eval_all group, usable is gqa_eval_all_tgt=True
# TODO: maybe with this norm
parser.add_argument('--wn_eval_all_norm', default='none', choices=['none', 'l1', 'l2'], help='normalize valuation_tgt')
# using cpu
parser.add_argument('--wn_eval_parallel_cpu', default=False, type=str2bool, help='parallelize evaluation')
parser.add_argument('--wn_eval_parallel_cpu_async', default=True, type=str2bool, help='useful if gqa_eval_parallel=True. parallelize evaluation asynchronously')


# eval all using gpu
# set --gqa_eval_only=True --gqa_eval_all_tgt=True --gqa_eval_each_tgt=False
parser.add_argument('--wn_eval_split_gpu', default=False, type=str2bool, help='parallelize evaluation in multi-gpu')
parser.add_argument('--wn_eval_split_gpu_total', default=6, type=int, help='the number of gpu to be used if parallelize evaluation in multi-gpu')
parser.add_argument('--wn_eval_split_gpu_id', default=0, type=int, help='the current id of split in dataset if parallelize evaluation in multi-gpu. Id starts from 0')
...
if __name__ == '__main__':
    ...
    if args.loss_tag=='default':
        args.loss_tag = f'ts{args.train_steps}_es{args.eval_steps}_template{args.template_set}_md{args.max_depth}_rec{args.recursivity}_iterPerRound{args.wn_iter_per_round}_numRound{args.wn_num_round}_mt{args.merging_tgt}_feat{args.num_feat}_gpu{args.use_gpu}_lrbg{args.wn_lr_bgs}_lri{args.wn_lr_its}_lrr{args.wn_lr_rules}_tgls{args.wn_tgt_ls}'
        # if args.wn_random_iter_per_round > 0:
        #     args.loss_tag += f'_randomIPP{args.wn_random_iter_per_round}'
